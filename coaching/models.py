...
class AssignmentCorrection(models.Model):
    WORK_TYPE_CHOICES = (
        ('notebook', 'Notebook Correction'),
        ('assignment', 'Assignment / Homework'),
        ('project', 'Project Work'),
        ('test', 'Class Test / Exam'),
    )
    STATUS_CHOICES = (
        ('submitted', 'Submitted for Correction'),
        ('verified', 'Verified & Returned to Student'),
        ('resubmit', 'Needs Resubmission'),
    )
    student = models.ForeignKey(StudentProfile, on_delete=models.CASCADE, related_name='corrections')
    subject = models.ForeignKey(Subject, on_delete=models.SET_NULL, null=True, blank=True)
    teacher = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name='verified_corrections')
    work_type = models.CharField(max_length=20, choices=WORK_TYPE_CHOICES, default='notebook')
    title = models.CharField(max_length=200, help_text="e.g. Chapter 4 Homework or Science Project")
    marks_obtained = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True)
    max_marks = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True, default=100.00)
...
class ClassroomMovementLog(models.Model):
    MOVEMENT_CHOICES = (
        ('IN', 'Entered Class'),
        ('OUT', 'Left Class'),
    )
    student = models.ForeignKey(StudentProfile, on_delete=models.CASCADE, related_name='movement_logs')
    teacher = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='recorded_movements')
    subject = models.ForeignKey(Subject, on_delete=models.SET_NULL, null=True, blank=True)
    movement_type = models.CharField(max_length=10, choices=MOVEMENT_CHOICES)
    reason = models.CharField(max_length=200, blank=True, null=True, help_text="e.g. Restroom, Lab visit, Home leave")
