...
@login_required
@role_required('teacher', 'super_admin')
def verify_correction_api(request):
    """API endpoint for teachers to record QR-scanned assignment, notebook correction, or project marks."""
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            qr_token = data.get('qr_token')
            ...
            correction = AssignmentCorrection.objects.create(
                ...
                work_type=work_type,
                title=title,
                marks_obtained=marks_obtained if (marks_obtained is not None and marks_obtained != "") else None,
                max_marks=max_marks if (max_marks is not None and max_marks != "") else 100,
                status=status,
            ...
            return JsonResponse({
                'success': True,
                'message': f"Successfully verified {correction.get_work_type_display()} for {student.user.get_full_name()}!",
...
@login_required
@role_required('teacher', 'super_admin')
def record_movement_api(request):
    """API endpoint for logging classroom IN/OUT entries and exits by scanning student QR."""
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            qr_token = data.get('qr_token')
            ...
            time_str = timezone.localtime(log_entry.timestamp).strftime('%I:%M %p')
            msg_action = "Left classroom" if movement_type == 'OUT' else "Returned to classroom"

            return JsonResponse({
                'success': True,
                'message': f"{student.user.get_full_name()} {msg_action} at {time_str}.",
                'student_name': student.user.get_full_name(),
                'movement_type': movement_type,
                'time': time_str
            })
